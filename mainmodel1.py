...
# تابع هدف: کم کردن تداخل درس‌های رشته + فاصله اسلات‌های استاد + توزیع نایکنواخت
# جریمه برای توزیع نایکنواخت
slot_load = pulp.LpVariable.dicts("slot_load", [s for s in range(len(time_slots))], lowBound=0, cat="Continuous")
for s in range(len(time_slots)):
    model += slot_load[s] == pulp.lpSum(x[t, s] for t in range(len(tasks)))
...
model += (
    ...
    pulp.lpSum(slot_load[s] * slot_load[s] for s in range(len(time_slots))) * 10  # جریمه توزیع نایکنواخت
), "Minimize_Objective"
