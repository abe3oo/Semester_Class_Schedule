...
SETTING_NO_OVERLAP_INPUT_HARD = False  # عدم تداخل زمانی تمام دروس یک ورودی (توصیه: False)
...
if SETTING_PREFER_CONSECUTIVE_TIME_SOFT:
    for prof, data in professors.items():
        if data['group'] in ['هیات_علمی', 'مدعو']:
            ...
            for day in days:
                # اگر در روزی فقط یک اسلات زمانی وجود دارد، نیازی به Gap نیست
                if len(day_time_map[day]) <= 1:
                    continue

                for i in range(1, len(day_time_map[day]) - 1):
                    # سه اسلات زمانی متوالی: قبل (i-1)، وسط (i)، بعد (i+1)
                    ...
                    # متغیر Gap: اگر کلاس قبلی و کلاس بعدی فعال باشند، اما کلاس وسط فعال نباشد، یک شکاف (Gap) ایجاد شده است.
                    gap_var = pulp.LpVariable(f"gap_{prof}_{day}_{i}", cat='Binary')

                    # 1. کلاس قبل را در اسلات قبلی تخصیص دهد
                    prev_assigned = pulp.lpSum(
                        assign_vars[key] for key in assign_vars if key[1] == prof and key[2] == prev_time)
                    # 2. کلاس بعد را در اسلات بعدی تخصیص دهد
                    next_assigned = pulp.lpSum(
                        assign_vars[key] for key in assign_vars if key[1] == prof and key[2] == next_time)
                    # 3. کلاس وسط را در اسلات جاری تخصیص ندهد (1 - current_assigned)
                    current_assigned_neg = 1 - pulp.lpSum(
                        assign_vars[key] for key in assign_vars if key[1] == prof and key[2] == curr_time)

                    # متغیرهای کمکی برای ساده سازی:
                    # prev_exists, next_exists, current_empty

                    gap_indicator = pulp.LpVariable(f"gap_ind_{prof}_{day}_{i}", cat='Binary')

                    # اگر کلاس قبلی و بعدی وجود داشته باشد
                    model += prev_assigned + next_assigned <= gap_indicator * 2 + 2, f"Prof_{prof}_Time_PrevNext_{day}_{i}"

                    # اگر کلاس وسط خالی باشد، جریمه اعمال شود
                    # متغیرهای کمکی برای تعیین اینکه آیا یک اسلات زمانی استفاده شده است یا خیر:
                    # is_used[time] = 1 اگر یک کلاس در این زمان باشد
                    is_used = lambda t: pulp.lpSum(
                        assign_vars[key] for key in assign_vars if key[1] == prof and key[2] == t)

                    # اگر اسلات قبلی و بعدی استفاده شده باشند، اما اسلات جاری نه، جریمه اعمال می‌شود
                    M_gap = 2  # چون فقط دو کلاس را چک می‌کنیم
...
print(
    f"تنظیمات فعال: Min Sec Soft={SETTING_MIN_SEC_SOFT}, Unified Prof Hard={SETTING_UNIFIED_PROFESSOR_HARD}, Consecutive Time Soft={SETTING_PREFER_CONSECUTIVE_TIME_SOFT}")
...
if status == pulp.LpStatusOptimal or status == pulp.LpStatusNotSolved:
    ...
    print("توجه: اگر مدل Infeasible شد، یکی از SETTING_..._HARD ها را FALSE کنید.")