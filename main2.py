...
def evaluate(individual):
    ...
    # بررسی تداخل درس‌های یه رشته (جریمه بیشتر)
    dept_conflicts = 0
    for slot in range(len(time_slots)):
        slot_tasks = [i for i, s in enumerate(individual) if s == slot]
        ...
        for dept in set(depts):
            if depts.count(dept) > 1:
                dept_conflicts += (depts.count(dept) - 1) * 100  # جریمه سنگین‌تر

    # توزیع یکنواخت
    slot_counts = [individual.count(slot) for slot in range(len(time_slots))]
    distribution_penalty = np.std(slot_counts) * 20  # جریمه قوی‌تر
...
def main():
    random.seed(42)
    pop = toolbox.population(n=200)  # افزایش اندازه جمعیت
    hof = tools.HallOfFame(1)

    stats = tools.Statistics(lambda ind: ind.fitness.values)
    stats.register("min", np.min)

    algorithms.eaSimple(pop, toolbox, cxpb=0.7, mutpb=0.2, ngen=200,  # افزایش نسل‌ها
